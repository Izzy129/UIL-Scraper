        ...
        while (++district <= 32) {
            try {
                ...
                if (cont) { // results
                    ...
                    // tableRows.slice(6, 7).each((index, element) => {
                    //     const rowData = [];
                    ...
                    // start at 7 just to get scores
                    var hitTeam = false;
                    tableRows.slice(7).each((index, element) => {
                        const rowData = [];
                        $(element).find('td').each((i, el) => {
                            if ($(el).text().trim() == "Place") { // hit team section, dont need this
                                hitTeam = true;
                            }
                            if (!hitTeam) rowData.push($(el).text().trim());
                        });
                        XLSX.utils.sheet_add_aoa(ws, [rowData], { origin: -1 });
                    });
                }
...
async function main() {
    ...
    XLSX.writeFile(wb, events.get("" + eventID) + " All Scores.xlsx");
