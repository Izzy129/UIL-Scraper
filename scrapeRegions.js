...
async function scrapeTableData(conference, eventID, seasonID) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    console.log("Scraping scores, this will take ~5 seconds");
    console.log();
    while (++region <= 4) {
      try {
        var url = 'https://postings.speechwire.com/r-uil-academics.php?groupingid=' + eventID + '&Submit=View+postings&region='+region+'&district=&state=&conference=' + conference + '&seasonid=' + seasonID;
        ...
        if (cont) { // results
          postedRegions.push(region); // for printing 
          ...
          // write to file
          XLSX.writeFile(wb, conference + "A " + events.get("" + eventID) + " Regional Scores.xlsx");
        }
      } catch (error) {
        console.error('Error scraping table data:', error);
      }
    }
    var posted = events.get("" + eventID) + " "  + conference + "A Posted Regions: ";
    var missing = events.get("" + eventID) + " " + conference + "A Missing Regions: ";
    missingRegions.forEach(d => {
      missing += d + ", ";
    });

    postedRegions.forEach(d => {
      posted += d + ", ";
    });
